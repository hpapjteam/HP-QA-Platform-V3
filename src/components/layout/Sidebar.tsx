...
export function Sidebar({ role }: { role: string }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("settings_profile_" + role);
    if (saved) {
  ...
  return (
    <aside 
      ...
      className={cn(
        "flex-shrink-0 bg-white text-slate-800 flex flex-col z-30 transition-all duration-300 relative border-r border-slate-200 shadow-xl",
        isCollapsed ? "w-20" : "w-64",
        isCollapsed && isHovered && "w-64 absolute left-0 top-0 bottom-0 shadow-2xl bg-white border-r border-slate-200"
      )}
    >
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-8 bg-white border border-slate-200 rounded-full p-1 text-slate-500 hover:text-white hover:bg-[#2b61d6] shadow-md z-40 cursor-pointer transition-colors"
        title={isCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
      >
        {isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
      </button>

      <div className={cn("p-6 pb-4 overflow-hidden whitespace-nowrap border-b border-black/10", !effectivelyExpanded ? "px-4" : "")}>
        <img 
      ...
      <div className={cn("p-4 flex flex-col gap-2 border-t border-black/10 bg-white/40", !effectivelyExpanded ? "items-center px-2" : "")}>
        <div className={cn("flex items-center rounded-xl bg-white/60 p-2 border border-black/5", !effectivelyExpanded ? "justify-center border-none bg-transparent p-0" : "space-x-3")}>
          ...
          {effectivelyExpanded && (
            <div className="overflow-hidden">
              ...
              <p className="text-[11px] font-medium text-slate-600 leading-none truncate">{profile.team}</p>
            </div>
          )}
        </div>
