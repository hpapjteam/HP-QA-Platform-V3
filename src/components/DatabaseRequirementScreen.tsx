...
import { Database, AlertTriangle, RefreshCw, Server, ExternalLink, CheckCircle2, Key } from "lucide-react";
        ...
        <div className="p-6 md:p-8 space-y-6 text-slate-300 text-sm">
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
            <span className="text-xs text-slate-500">
              Once environment variables are updated, reload page to continue.
            </span>
            <button
              onClick={handleRefresh}
              disabled={checking}
              className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-5 py-2.5 rounded-xl text-xs transition-all flex items-center gap-2 shadow-lg disabled:opacity-50 cursor-pointer"
            >
              <RefreshCw className={`w-4 h-4 ${checking ? "animate-spin" : ""}`} />
              {checking ? "Checking Connection..." : "Check Connection & Reload"}
            </button>
          </div>
        </div>
      </div>
