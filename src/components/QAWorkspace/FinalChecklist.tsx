...
export function FinalChecklist({ checklists, answers }: FinalChecklistProps) {
  ...
  const stages = [1, 2, 3, 4, 5, 6, 0];
  const stageNames = ["Details & Source", "Visual Comparison", "Alt & Alias Tags", "Link Validation", "Grammar & Spell Check", "Review & Decision"];
            ...
            <Card key={stageNum} className="border-slate-200 shadow-sm overflow-hidden">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100 py-3 px-4">
                <CardTitle className="text-sm text-slate-700 flex items-center gap-2">
                  ...
                  {stageNum === 0 ? "Global Checkpoints (All Stages)" : `Step ${stageNum}: ${stageNames[stageNum - 1]}`}
                </CardTitle>
              </CardHeader>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.requiresInput && isChecked && ans.text && (
                            <div className="mt-1.5 p-2 bg-indigo-50/50 border border-indigo-100 rounded text-sm text-indigo-900">
                              <span className="font-semibold text-indigo-700 mr-2">Input:</span> 
                              {ans.text}
                            </div>
                          )}
                        </div>
