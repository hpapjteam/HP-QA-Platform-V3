...
export function RecycleBin({ userEmail = "admin@example.com", userRole }: { userEmail?: string; userRole?: string }) {
  ...
  const handlePermanentDelete = async (campaign: CampaignRecord) => {
    try {
      ...
      await logAction(userEmail, "Permanent Delete", `Permanently deleted campaign "${campaign.name}" from Recycle Bin`, String(campaign.id));
      ...
      setToastNotice(`⚠️ Failed to permanently delete campaign "${campaign.name}".`);
      setTimeout(() => setToastNotice(null), 4000);
    }
  };
  ...
  const handleEmptyBin = async () => {
    ...
    try {
      ...
      setToastNotice(`⚠️ Failed to empty Recycle Bin.`);
      setTimeout(() => setToastNotice(null), 4000);
    } finally {
